...
namespace InventoryManagerIntegrationTests
{
    ...
    {
        private IItemRepos _dbRepo;
        DbContextOptions<InventoryManageDbContext> _options;
        ...
        {
            _options = new DbContextOptionsBuilder<InventoryManageDbContext>()
                .UseInMemoryDatabase(databaseName: "InventoryManagerTest").Options;
        ...
        {
            using (var context = new InventoryManageDbContext(_options))
            {
                ...
                if (item1Detail != null && item2Detail != null && item3Detail != null)
                    return; var color1 = new CategoryDetail()
                    {
        ...
        public void TestGetItems()
        {
            ...
            using(var context = new InventoryManageDbContext(_options))
            {
                _dbRepo = new ItemRepos(context, _mapper);
                var items = _dbRepo.GetItems();
                items.ShouldNotBeNull(); items.Count.ShouldBe(3);
        ...
        public void TestCategoryColors(string name, string color, string colorValue)
        {
            ...
            {
                // act
                var categoriesRepo = new CategoryRepos(context, _mapper);
